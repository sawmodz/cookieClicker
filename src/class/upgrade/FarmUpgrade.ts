...
export default class CursorUpgrade implements Upgrade {
  ...
  constructor() {
    this.bonus = 0;
    this.upgradeBaseCost = 1100;
    this.realBaseCost = 1100;
  ...
  showUpgrade = (): void => {
    upgradeContainer?.insertAdjacentHTML(
      "beforeend",
      `
    <div class="upgrade-component" id="farm-upgrade">
      <p class="upgrade-name">🚜 Ferme (niveau ${this.currentLevel})</p>
      <p>Augmente la production de cookies de ${parseFloat(
        this.bonus.toString()
      ).toFixed(1)} par secondes</p>
      <button class="upgrade-button" id="farm-upgrade-button">
  ...
  getIdButton(): string {
    return "farm-upgrade-button";
  }

  refreshBox(): void {
    const cursorUpgrade = document.getElementById("farm-upgrade");
  ...
  saveAllData(): void {
    localStorage.setItem("farmUpgrade", JSON.stringify(this));
  }

  loadAllData(): void {
    const cursorUpgrade = localStorage.getItem("farmUpgrade");
